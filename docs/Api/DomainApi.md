...
[**\PidginHost\Sdk\Model\DNSGlue**](../Model/DNSGlue.md)
...
```php
domainDomainDnsDestroy($domain, $name)
```
...
try {
    $apiInstance->domainDomainDnsDestroy($domain, $name);
} catch (Exception $e) {
    echo 'Exception when calling DomainApi->domainDomainDnsDestroy: ', $e->getMessage(), PHP_EOL;
}
```
...
```php
domainDomainDnsList($domain, $page): \PidginHost\Sdk\Model\PaginatedDNSGlueList
```
...
try {
    $result = $apiInstance->domainDomainDnsList($domain, $page);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling DomainApi->domainDomainDnsList: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\PidginHost\Sdk\Model\PaginatedDNSGlueList**](../Model/PaginatedDNSGlueList.md)
